...
class Realtimedata_model extends CI_Model {
    ...
    public function get_energy_graph() 
    {
        ...
        if(!strncmp($period, "weekly", 6))
        {
            ...
            $dsn = 'sqlite:'.APPPATH.'../../../historical_data.db';
            $this->pdo = new PDO($dsn);
            $query = "SELECT date,daily_energy FROM daily_energy WHERE date BETWEEN $query_date_start AND $query_date_end ORDER BY date";
            $result = $this->pdo->query($query);
            if(empty($result))return $data;//没有数据库或SQL语句错误            
            $res = $result->fetchAll(PDO::FETCH_ASSOC);
        ...
        else if(!strncmp($period, "yearly", 6))
        {
            ...
            $dsn = 'sqlite:'.APPPATH.'../../../historical_data.db';
            $this->pdo = new PDO($dsn);
            $query = "SELECT date,monthly_energy FROM monthly_energy WHERE date BETWEEN $query_date_start AND $query_date_end  ORDER BY date";           
            $result = $this->pdo->query($query);
            if(empty($result))return $data;//没有数据库或SQL语句错误
            $res = $result->fetchAll(PDO::FETCH_ASSOC);
